...
class RandeerSampler(Sampler):
    ...
    def init_in_interval_excluding_task(self, id_: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.init_in_interval_excluding_task(id_, min_, max_, encode_list(excluded), len(excluded))
    ...
    def sample_default_by_looping(self, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_by_looping(min_, max_, encode_list(excluded), len(excluded))

    def sample_default_n_by_looping_without_init(self, n: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_n_by_looping_without_init(n, min_, max_, encode_list(excluded), len(excluded))

    def sample_default_n_by_looping_with_init(self, n: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_n_by_looping_with_init(n, min_, max_, encode_list(excluded), len(excluded))

    # default looping using objects

    def sample_default_by_looping_using_objects(self, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_by_looping_using_objects(min_, max_, encode_list(excluded), len(excluded))

    def sample_default_n_by_looping_without_init_using_objects(self, n: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_n_by_looping_without_init_using_objects(n, min_, max_, encode_list(excluded), len(excluded))

    def sample_default_n_by_looping_with_init_using_objects(self, n: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_n_by_looping_with_init_using_objects(n, min_, max_, encode_list(excluded), len(excluded))

    # default shifting using objects

    def sample_default_by_shifting_using_objects(self, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_by_shifting_using_objects(min_, max_, encode_list(excluded), len(excluded))

    def sample_default_n_by_shifting_without_init_using_objects(self, n: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_default_n_by_shifting_without_init_using_objects(n, min_, max_, encode_list(excluded), len(excluded))
    ...
    def sample_lcg_by_looping(self, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_lcg_by_looping(min_, max_, encode_list(excluded), len(excluded))

    def sample_lcg_n_by_looping_without_init(self, n: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_lcg_n_by_looping_without_init(n, min_, max_, encode_list(excluded), len(excluded))

    def sample_lcg_n_by_looping_with_init(self, n: int, min_: int, max_: int, excluded: Tuple[int]):
        return self.lib.sample_lcg_n_by_looping_with_init(n, min_, max_, encode_list(excluded), len(excluded))
        ...
        if sampling_approach == SamplingApproach.DEFAULT:
            ...
            if sampling_method == SamplingMethod.SHIFTING:
                if using_objects:
                    if iteration_method == IterationMethod.PYTHON:
                        for _ in range(n):
                            _ = self.sample_default_by_shifting_using_objects(min_, max_, excluded)
                        return
                    if single_init:
